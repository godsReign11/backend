...
const sentPhoneOtp = async (key, type) => {
    ...
    if (otpResponse?.data?.type === "success") {
        if (type === "register") {
            await appotp.updateOne({ phone, description: "Phone Register OTP!" }, { otp }, { upsert: true });
            return true;
        } else if (type === "login") {
            await appotp.updateOne({ phone, description: "Phone Login OTP!" }, { otp }, { upsert: true });
            return true;
        } else if (type === "forgot") {
            await appotp.updateOne({ phone, description: "Phone ForgotPassword OTP!" }, { otp }, { upsert: true });
            return true;
        }
    } else {
        return false;
